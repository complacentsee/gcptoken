...
String RESTHost = "us-central1-gcptoken.cloudfunctions.net";
int RESTPort = 443;
String RESTPath = "/gcptoken_test_endpoint";
...
// the content of this JSON does not matter for the test as it is not checked
...
void RESTPost(String msg, String BearerToken) {
  ...
  if (clientREST.connect(RESTHost.c_str(), RESTPort)) {
    clientREST.setCertificate(root_gcp_cert);
    HTTPClient https_rest; 

      if (https_rest.begin(clientREST, RESTHost, RESTPort, RESTPath, true)) {
          https_rest.addHeader("Authorization", String("Bearer ") + BearerToken);
          https_rest.addHeader("Host", String(RESTHost));
          https_rest.addHeader("Content-Type", "application/json");
          https_rest.addHeader("Content-Length", String(msg.length()));

          int  httpCode = https_rest.POST(msg);
          if (httpCode > 0 && httpCode < 400 ) {
            ...
            if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
              String payload = https_rest.getString();
              Serial.println(payload);
            }
          } else {
            Serial.println("[HTTPS] failed, error: " + String(httpCode) + " = " +  https_rest.errorToString(httpCode).c_str());
          }
          https_rest.end();
        } else {
